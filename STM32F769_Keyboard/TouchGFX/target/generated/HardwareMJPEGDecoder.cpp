...
uint32_t JPEG_IN_BufferIndex = 0;
uint32_t JPEG_OUT_BufferIndex = 0;
...
/********** TODO **************/
//Just do one input buffer for now? Or, create function in MJPEGDecoder to get address of next frame.
//Output buffer is for RGB data. Use temp buffer for conversion.
void HardwareMJPEGDecoder::decodeMJPEGFrame(const uint8_t* const mjpgdata, const uint32_t length, uint8_t* outputBuffer, uint16_t bufferWidth, uint16_t bufferHeight, uint32_t bufferStride)
{
  //TODO: Must block here until decoding of the frame is done
    if (length == 0)
    {
        return;
    }
...
//Data has been converted from JPEG to YCbCr. Now convert it using convert function into the
